...
function lerUmaNoticia($conexao, $idNoticia, $idUsuario, $tipoUsuario)
{
    if ($tipoUsuario === 'admin') {
        ...
        //Senão, pode carregar/ver dados SOMENTE DAS SUAS PROPRIAS noticias
...
function atualizarNoticia(
    ...
    if ($tipoUsuario === 'admin') {
        //Pode atualizar QUALQUER noticia desde que ELE SAIBA QUAL
        $sql = "UPDATE noticias SET
        titulo = '$titulo', texto = '$texto',
        resumo = '$resumo', imagem = '$imagem'
        WHERE id = $idNoticia"; // NAO ESQUECE DE USAR WHERE
...
function buscar($conexao, $termoDigitado)
{
    ...
    /* Atencao: usamos o LIKE em vez de igual no WHERE
    e tambem o % para ampliar a possibilidade de encontrar
    o termo digitado no banco de dados.
    Usamos o LIKE com % para uma persquisa parcial/nao exata.*/
