...
//Chamamos a funçao para carregar os dados da pessoa através do id
...
if (isset($_POST['atualizar'])) {
	...
	if (empty($_POST['senha']) || password_verify($_POST['senha'], $dadosUsuario['senha'])) {
		//Manter a mesma senha(copiamos a senha do banco para uma variavel)
		$senha = $dadosUsuario['senha'];
	} else {
		//Codificar a nova senha digitada no formulário
		$senha = password_hash($_POST['senha'], PASSWORD_DEFAULT);
	}
	//Chamamos a funçao para o UPDATE no banco

	atualizarUsuario($conexao, $id, $nome, $email, $senha, $tipo);

	//Redirecionamos para a página com a lista de usuários
